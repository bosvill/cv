...
const createCv = async data => {
  ...
  // await User.findByIdAndUpdate(data.user,{cvs:result._id})
  return result
}
...
const getAllCvs = async id => {
  const cvs = await CV.find({ user: id }).select('createdAt firstName lastName template')
  console.log('GetAllCvs service cvs: ', cvs)
  return cvs
}
